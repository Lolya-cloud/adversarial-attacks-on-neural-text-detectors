...
results_path = "C:/Users/vital/PycharmProjects/M12Project/basic_prompt_engineering_results"
...
def check_assumptions(data1, data2, detector_name):
    ...
    plt.figure(figsize=(5, 4))
    sns.boxplot(data=[data1, data2], notch=False)
...
def plot_histograms(scores, detector_names, score_types):
    ...
    plt.title('"Smart prompt" vs "Baseline"')
...
def plot_boxplots(scores, detector_names, score_types):
    for detector in detector_names:
        ...
        plt.ylabel('Scores (0-100 scale)')
        plt.title(f'{detector.capitalize()} "Smart prompt" vs "Baseline"')
